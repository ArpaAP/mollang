...
### 반복문 (은?행 돌!자) _(이 기능은 1.2부터 지원됩니다.)_

특정 조건이 0이 될 때까지 반복을 진행합니다.

`{조건식}은?행{실행할 코드}돌!자`의 형태로 사용됩니다. 반복문이 종료되면 `돌!자`의 다음 부분으로 이동하게 됩니다.

### 함수 선언 (은?행 짓!자) _(이 기능은 1.2부터 지원됩니다.)_

`{함수명}은?행(P1),(P2),(P3) {실행할 코드}짓!자`의 형태로 사용됩니다.

함수의 파라미터 개수에는 제한이 없습니다. 파라미터 수가 하나일 경우 쉼표를 적지 않습니다.

단, 함수명은 초성이 ㅁ, ㄹ인 단어만 허용됩니다. 예: `말랑, 머리, 무릎, 망령, 매립, 무리, 밀랍, 문란, 뭵뤩  `

단, ㅁ의 자리에 몰, ㄹ의 자리에 루는 사용할 수 없습니다. 예: `몰랑, 모루`

### 함수 호출 (은?행 가!자) _(이 기능은 1.2부터 지원됩니다.)_

`{함수명}은?행(P1),(P2),(P3)가!자`의 형태로 사용됩니다.
